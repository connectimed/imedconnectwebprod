...
const page = () => {
  ...
  const sendMessage = async () => {
    if (name.length < 3) {
      ...
      setError("Enter valid subject!");
      setTimeout(() => {
        setError("");
      }, 2000);
    } else if (message.length < 12) {
      setError("Enter a long message!");
      setTimeout(() => {
        setError("");
      }, 2000);
    } else {
  ...
  return (
    <div className="bg-primary-deep-dark h-screen pt-6">
      <div class="mt-0 max-w-6xl max-lg:max-w-3xl mx-auto bg-primary-dark rounded-lg">
        <div class="grid lg:grid-cols-2 items-center gap-14 sm:p-8 p-4 font-[sans-serif]">
          <div>
            ...
            <ul class="flex mt-12 space-x-4">
              <li class="bg-primary-light hover:bg-primary-deep-dark h-10 w-10 rounded-full flex items-center justify-center shrink-0">
          ...
          <div class="bg-gray-100 p-6 rounded-lg">
            ...
            {/* <div class="space-y-4 max-lg:mt-4">
              <button
                type="button"
                class="px-4 py-2 rounded-lg bg-primary-light text-white text-sm tracking-wider font-medium outline-none border-2 border-gray-300 mr-4"
              >
                Mentorship
              </button>
              <button
            ...
            <form class="mt-4 space-y-4">
              <input
                type="text"
                placeholder="Name"
                class="w-full rounded-lg py-3 px-4 text-gray-800 text-sm outline-primary-light"
                value={name}
              ...
              <textarea
                placeholder="Message"
                rows="6"
                class="w-full rounded-lg px-4 text-gray-800 text-sm pt-3 outline-primary-light"
                value={message}
                onChange={handleMessageChange}
              ></textarea>
              {error && <ErrorBody error={error} />}
              {success && <SuccessBody text={success} />}
              <button
                type="button"
                class="text-white bg-primary-light hover:bg-primary-dark tracking-wide rounded-lg text-sm px-4 py-3 flex items-center justify-center w-full !mt-6"
